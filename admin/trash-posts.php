...
// Restore Post
if(isset($_GET['action']) && $_GET['action']=='restore' && isset($_GET['pid'])){
...
// Permanent Delete
if(isset($_GET['action']) && $_GET['action']=='perdel' && isset($_GET['presid'])){
    $id = intval($_GET['presid']);
    $query = mysqli_query($con,"DELETE FROM tblposts WHERE id='$id' LIMIT 1");
    if($query){
        $delmsg = "Post deleted forever";
    } else {
        $error = "Something went wrong while deleting permanently.";    
    }
}
